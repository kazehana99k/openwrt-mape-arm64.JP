...
fleth_style.innerHTML = `
  ...
  .ppe-status-good { color: #0088cc; font-weight: bold; }
  .ppe-status-bad { color: #F44336; font-weight: bold; }
`;
...
function ppeEnabled(ppeStatus) {
  return ppeStatus.enabled === "1" && ppeStatus.accel_ipip6 === "1";
}

function ppeBoolHtml(value) {
  const enabled = value === "1";

  return '<span class="' + (enabled ? 'ppe-status-good' : 'ppe-status-bad') + '">' +
...
return view.extend({
  ...
  load: function () {
    return Promise.all([
      ...
      L.resolveDefault(fs.exec("/usr/sbin/fleth", ["ppe_status"]), { stdout: "" }),
      ...
      const ppe_status = parseKeyValueLines(results[4].stdout || "");
  ...
  render: async function (data) {
    ...
    o.write = function(section_id, value) {
      uci.set("fleth", section_id, "enabled", value);
      if (value === "1")
        return fs.exec("/usr/sbin/fleth", ["autoconfigure"]);
    };
    ...
    o = s.taboption(
      ...
      "ppe_enabled",
      _("PPE Acceleration"),
      _("Enable Qualcomm PPE acceleration for the MAP-E tunnel.")
    );
    o.rmempty = false;
    o.default = "1";
    o.cfgvalue = function () {
      return ppeEnabled(data.ppeStatus) ? "1" : "0";
    };
    o.write = L.bind(function(section_id, value) {
      uci.set("fleth", section_id, "ppe_enabled", value);
      return this.setPpeAcceleration(value === "1");
    }, this);
    ...
    o = s.taboption("tools", form.DummyValue, "_ppe_status");
    o.title = _("PPE Acceleration");
    o.rawhtml = true;
    o.cfgvalue = function () {
      const p = data.ppeStatus || {};

      return [
        _("Configured") + ": " + ppeBoolHtml(p.enabled),
        _("ipip6 acceleration mode") + ": " + ppeBoolHtml(p.accel_ipip6),
        _("MAP-E Interface") + ": " + (p.mape_up === "1" ? _("Up") : _("Down")),
        _("PPE VP") + ": " + (p.ppe_vp || "-"),
        _("ECM PPE IPv4 accelerated count") + ": " + (p.ecm_ppe_ipv4 || "0")
      ].join("<br />");
    };

    o = s.taboption("tools", form.Button, "_enable_ppe");
    o.title = "&#160;";
    o.inputtitle = _("Enable PPE acceleration");
    o.inputstyle = ppeEnabled(data.ppeStatus) ? "cbi-button-action" : "cbi-button-apply";
    o.onclick = L.bind(function () {
      return this.setPpeAcceleration(true);
    }, this);

    o = s.taboption("tools", form.Button, "_disable_ppe");
    o.title = "&#160;";
    o.inputtitle = _("Disable PPE acceleration");
    o.inputstyle = "cbi-button-action";
    o.onclick = L.bind(function () {
      return this.setPpeAcceleration(false);
    }, this);
  ...
  setPpeAcceleration: function (enabled) {
    return new Promise(function (resolve, reject) {
      ui.showModal(enabled ? _('Enabling PPE acceleration') : _('Disabling PPE acceleration'), [
        E('p', { 'class': 'spinning' }, _('Please wait...'))
      ]);

      fs.exec('/usr/sbin/fleth', [enabled ? 'ppe_enable' : 'ppe_disable'])
        .then(function (result) {
          ui.hideModal();
        ...
        .catch(function (error) {
          ui.hideModal();
          ui.addNotification(null, E('div', [
            E('p', _('Error applying PPE setting:')),
            E('pre', error.message || error)
          ]), 'error');
          reject(error);
        });
